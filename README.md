...
2. Agrege comentarios al código actual. Documente las funciones y clases que no tengan comentarios adecuados. Para ello cree un nuevo archivo en este proyecto llamado `pixmatch_refactor.py` que sea una copia del original. Sobre este nuevo archivo hará los ajustes siguientes. **Tenga encuenta el nuevo nombre para cuando quiera ejecutar el proyecto**.
3. Cree un diagrama de flujo que documente el proceso de juego que el usuario da clic en el botón para iniciar el juego hasta que el usuario selecciona una imagen y el sistema verifica si la imagen seleccionada es o no correcta
     > Agregue el diagrama en la carpeta docs con el nombre `proceso_juego`
3. Agregue en el archivo [requisitos.md](docs/a.requisitos) la lista de requisitos (funcionalidades) que soporta el proyecto actualmente.Trate de identificar las funcionalidades principales que soporta el juego a partir de su revisión del código fuente, del uso del juego y de la revisión de la documentación que encuentre.   
...
4. Relacione fragmentos del código fuente, variables, etc con los requisitos que identifcó sirven para resolver el juego. 
Pueden ajustar la tabla para que se ajuste a los requisitos que identificaron en el análisis.
...
5. Modifique las siguientes funcionalidades, pruébelas para verificar que su cambio no afecto negativamente nada del funcionamiento ya existente. 
   ...
   * Perder el juego: El usuario puede fallar como máximo al 50% de las imágenes que tiene el tablero. Por ejemplo, un tablero fácil tiene 36 turnos. El usuario solo puede fallar 18 veces. Cuando complete la vez número 19, el jugador habrá perdido y no podrá jugar más. Para seguir jugando el usuario deberá volver a la página principal e iniciar un nuevo juego.   
...
Diligencie la [lista de chequeo](docs/d.lista_chequeo) a partir de su revisión como *desarrollador experto* del código fuente actual. Considere que usted trabaja en una empresa desarrolladora de software como _desarrollador senior_ y que es su responsabilidad asegurar que el código desarrollado por la empresa cumple con las mejores prácticas. 
  * Guardar **cuatro** en lugar de tres jugadores en el archivo que almacena el top de puntuación
   * Mostrar los cuatro principales jugadores y no sólo tres en la intefaz gráfica.
   * Perder el juego: El usuario puede fallar como máximo al 50% de las imagenes que tiene el tablero. Por ejemplo, un tablero fácil tiene 36 turnos. El usuario solo puede fallar 18 veces. Cuando complete la vez número 19, el jugador habrá perdido y no podrá jugar más. Para seguir jugando el usuario deberá volver a la página principal e iniciar un nuevo juego.
...
Reflexione sobre cómo podría estructurar el código usando clases para mejorar la claridad y mantenibilidad del código. Dado que todavía no tiene tanta experiencia con la programación en Streamlit, imagine que va a reestructurar su aplicación para que funcione en C++. 
    > Documente sus ideas en el archivo de [mejoras.md](docs/d.mejoras). Estas ideas serán discutidas en la próxima sesión.
